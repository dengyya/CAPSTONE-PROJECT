...
if ($status > 1) {
	$aqry =  $conn->query("SELECT * FROM crime_action where complaint_id = {$_GET['id']} ");
...
<div class="container-fluid">
	<div class="col-lg-12">
		<large><b>Type of Crime:</b></large>
		<p><?php echo $type_of_crime ?></p>
		<hr>
		<large><b>Location:</b></large>
		<p><?php echo $crime_street ?>, <?php echo $crime_barangay ?>(Landmark: <?php echo $crime_landmark ?>)</p>
		<large><b>Person involved:</b></large>
		<p><?php echo $involved_person ?></p>
		<hr>


		<form id="manage-crime">
			<div id="msg"></div>
			...
			<div class="assign-responder" style="display:none">
				<div class="form-group">
					...
					<select name="responder_id" id="" class="custom-select input-sm select2">
						<option value=""></option>
						<?php
						$complaints = $conn->query("SELECT rt.*,s.name as sname,s.address FROM responders_team rt inner join 
							stations s on s.id = rt.station_id where rt.availability = 1 " . (isset($ca['responder_id']) ? "
...
<script>
	$('#manage-crime').on('reset', function() {
		$('#msg').html('')
		$('input:hidden').val('')
	})
	$('.select2').select2({
	...
	$('#manage-crime').submit(function(e) {
		...
		$.ajax({
			url: 'ajax.php?action=manage_crime',
			method: 'POST',
			data: $(this).serialize(),
			error: err => {
				console.log(err)
			...
			success: function(resp) {
				if (resp == 1) {
					...
					$('#msg').html('<div class="alert alert-danger">Report Crime is not received yet.</div>')
					end_load()
				}
			}
		})
